...
public class TourStopSelectCommand extends BaseCommand {

    public TourStopSelectCommand() {
        super("select", "<id_tour> <id_parada>", "btecs.command.tour", CommandMode.PLAYER_ONLY);
    ...
    protected boolean onCommand(org.bukkit.command.CommandSender sender, String[] args) {
        ...
        RegionUtils.selectPolygon(bukkitPlayer, parada.getPoligono(), playerY - config.getInt("world-edit.region-select-extension"), playerY + config.getInt("world-edit.region-select-extension"), language);
        return true;
    }
