...
public class UPaisDsIdCountryChatCommand extends BaseCommand {

    private final YamlConfiguration lang;
    private final DBManager dbManager;

    public UPaisDsIdCountryChatCommand() {
        super("dsidcountrychat", "Actualizar Discord Country Chat ID de un País.", "<id> <nuevo_ds_id_country_chat>", CommandMode.BOTH);
    ...
    protected boolean onCommand(CommandSender sender, String[] args) {
        ...
        pais.setDsIdCountryChat(nuevoId);
        dbManager.merge(pais);

        String message = lang.getString("crud-update").replace("%entity%", "Pais").replace("%property%", "ds_id_country_chat");
        sender.sendMessage(message);
        return true;
    }
}
