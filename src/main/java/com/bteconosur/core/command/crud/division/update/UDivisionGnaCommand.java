...
public class UDivisionGnaCommand extends BaseCommand {
    ...
    public UDivisionGnaCommand() {
        super("gna", "<id> <nuevo_gna>", "btecs.command.crud", CommandMode.BOTH);
    ...
    protected boolean onCommand(CommandSender sender, String[] args) {
        ...
        if (nuevoTipo.length() > 100) {
            String message = LanguageHandler.getText(language, "crud.not-valid-type").replace("%entity%", "Division").replace("%type%", nuevoTipo).replace("%reason%", "Máximo 100 caracteres.");
            PlayerLogger.error(sender, message, (String) null);
            return true;
        }
