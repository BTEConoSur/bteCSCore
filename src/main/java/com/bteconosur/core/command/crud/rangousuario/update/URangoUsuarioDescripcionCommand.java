...
public class URangoUsuarioDescripcionCommand extends BaseCommand {
    ...
    public URangoUsuarioDescripcionCommand() {
        super("descripcion", "<id> <descripcion>", "btecs.command.crud", CommandMode.BOTH);
    ...
    protected boolean onCommand(CommandSender sender, String[] args) {
        ...
        if (nuevaDescripcion.length() > 500) {
            String message = LanguageHandler.getText(language, "crud.not-valid-name").replace("%entity%", "Rango de Usuario").replace("%name%", nuevaDescripcion).replace("%reason%", "Máximo 500 caracteres.");
