...
public class UPlayerIdCommand extends BaseCommand{
    ...
    public UPlayerIdCommand() {
        super("id", "<uuid> <valor>", "btecs.command.crud", CommandMode.BOTH);
    ...
    protected boolean onCommand(CommandSender sender, String[] args) {
        ...
        try {
            ...
            String message = LanguageHandler.getText(language, "crud.not-valid-id").replace("%entity%", "Player").replace("%id%", args[1]);
            PlayerLogger.error(sender, message, (String) null);
            return true;
        }
        ...
        if(dbManager.exists(Player.class, newUuid)) {
            String message = LanguageHandler.getText(language, "crud.already-exists").replace("%entity%", "Player").replace("%id%", args[1]);
            PlayerLogger.error(sender, message, (String) null);
            return true;
        }
