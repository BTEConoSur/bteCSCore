...
public class UTipoProyectoNombreCommand extends BaseCommand {
    ...
    public UTipoProyectoNombreCommand() {
        super("nombre", "Actualizar nombre de un TipoProyecto.", "<id> <nombre>", CommandMode.BOTH);
    ...
    protected boolean onCommand(CommandSender sender, String[] args) {
        ...
        if (nuevoNombre.length() > 50) {
            String message = lang.getString("crud-not-valid-name").replace("%entity%", "TipoProyecto").replace("%name%", nuevoNombre).replace("%reason%", "Máximo 50 caracteres.");
            PlayerLogger.error(sender, message, (String) null);
            return true;
        }
