...
public class UPlayerTipoUsuarioCommand extends BaseCommand {
    ...
    protected boolean onCommand(CommandSender sender, String[] args) {
        ...
        try {
            ...
            String message = LanguageHandler.getText(language, "crud.not-valid-parse").replace("%entity%", "TipoUsuario").replace("%value%", args[1]).replace("%type%", "Long");
            PlayerLogger.error(sender, message, (String) null);
            return true;
        }
        ...
        if (!dbManager.exists(TipoUsuario.class, tipoId)) {
            String message = LanguageHandler.getText(language, "crud.read-not-found").replace("%entity%", "TipoUsuario").replace("%id%", args[1]);
            PlayerLogger.error(sender, message, (String) null);
            return true;
        }
