...
public class URangoUsuarioCommand extends BaseCommand {
    ...
    public URangoUsuarioCommand() {
        super("rangousuario", "<uuid> <id_rango_usuario>", "btecs.command.crud", CommandMode.BOTH);
    ...
    protected boolean onCommand(CommandSender sender, String[] args) {
        ...
        try {
            ...
            String message = LanguageHandler.getText(language, "crud.not-valid-parse").replace("%entity%", "RangoUsuario").replace("%value%", args[1]).replace("%type%", "Long");
            PlayerLogger.error(sender, message, (String) null);
            return true;
        }
        ...
        if (!dbManager.exists(RangoUsuario.class, rangoId)) {
            String message = LanguageHandler.getText(language, "crud.read-not-found").replace("%entity%", "RangoUsuario").replace("%id%", args[1]);
            PlayerLogger.error(sender, message, (String) null);
            return true;
        }
