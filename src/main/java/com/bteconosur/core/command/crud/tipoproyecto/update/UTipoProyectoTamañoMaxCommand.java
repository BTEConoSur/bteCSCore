...
public class UTipoProyectoTamañoMaxCommand extends BaseCommand {
    ...
    public UTipoProyectoTamañoMaxCommand() {
        super("tamañomax", "Actualizar tamaño máximo de un TipoProyecto.", "<id> <tamaño>", CommandMode.BOTH);
    ...
    protected boolean onCommand(CommandSender sender, String[] args) {
        ...
        try {
            ...
            String message = lang.getString("crud-not-valid-parse").replace("%entity%", "tamaño_max").replace("%value%", args[1]).replace("%type%", "Integer");
            PlayerLogger.error(sender, message, (String) null);
            return true;
        }
        ...
        if (nuevoTamano < tipoProyecto.getTamanoMin()) {
            String message = lang.getString("crud-not-valid-name").replace("%entity%", "TipoProyecto").replace("%name%", "Tamaño máximo").replace("%reason%", "No puede ser menor al tamaño mínimo.");
            PlayerLogger.error(sender, message, (String) null);
            return true;
        }
