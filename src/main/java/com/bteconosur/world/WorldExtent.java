...
public class WorldExtent extends AbstractDelegateExtent {
    ...
    {
        if (!worldManager.canBuild(new Location(world, location.getBlockX(), location.getBlockY(), location.getBlockZ()), player)) return false;
    ...
    public int setBlocks(final @NotNull Set<BlockVector3> vset, final Pattern pattern) {
        Set<BlockVector3> allowedBlocks = new java.util.HashSet<>();
        for (BlockVector3 position : vset) {
            if (worldManager.canBuild(new Location(world, position.getBlockX(), position.getBlockY(), position.getBlockZ()), player)) allowedBlocks.add(position);
        }
        return super.setBlocks(allowedBlocks, pattern);
    }
